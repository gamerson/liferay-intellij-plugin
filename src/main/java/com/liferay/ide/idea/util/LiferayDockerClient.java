...
import static org.apache.commons.lang.SystemUtils.IS_OS_WINDOWS_7;
...
public class LiferayDockerClient {
	...
	private static String _getDefaultDockerUrl() {
		...
		if (CoreUtil.isNullOrEmpty(dockerUrl)) {
			if (IS_OS_UNIX && new File("/var/run/docker.sock").exists()) {
				dockerUrl = new String("unix:///var/run/docker.sock");
			}
			else {
				if (IS_OS_WINDOWS && new File("\\\\.\\pipe\\docker_engine").exists()) {
					if (IS_OS_WINDOWS_7) {
						try {
